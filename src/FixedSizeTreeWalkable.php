 ...
 */
class FixedSizeTreeWalkable implements ITreeNode
{
    /**
     * @string[]
     */
    ...
    private static function buildHtreeRow($width, callable $hasher)
    {
        $row = [];
        if ($width === 2) {
            $row[] = new TwoChildrenNodeWalkable($hasher);
            return $row;
        }
        ...
        if ($width === 3) {
            $row[] = new TwoChildrenNodeWalkable($hasher);
            $row[] = new TwoChildrenNodeWalkable($hasher);
            $row[] = new OneChildDuplicateNodeWalkable(new TwoChildrenNodeWalkable($hasher));
        ...
        if ($odd) {
            $row[$rowSize - 1] = new OneChildDuplicateNodeWalkable($row[$rowSize - 1]);
        }
